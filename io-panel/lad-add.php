<?php
	...
	if(isset($_GET['edit'])){
		...
		$data = $admin->getUniqueLadderById($id);
	}

	if(isset($_POST['id']) && !empty($_POST['id'])) {
		if($csrf->check_valid('post')) {
			$id = trim($admin->escape_string($admin->strip_all($_POST['id'])));
			$result = $admin->updateLadder($_POST);
							...
							<div class="card">
								...
								<div class="card-body">
									...
									<div class="form-group row">
										<div class="col-md-4">
											<label>Key Benifit<em>*</em></label>
											<input type="text" name="key_name" value="<?php if(isset($_GET['edit'])) { echo $data['key_name']; } ?>" class="form-control" required />
										</div>
										<div class="col-sm-4">
											<label>Ladder Category</label>
											<select autocomplete="off" class="form-control select" name="ladder[]" multiple placeholder="Select Asubscription Products">
												<?php
													$ladderArray = array();
													if(isset($_GET['edit'])){
														$ladderArray = explode(",", $data['ladder']);
													}
													$ladderSQL = $admin->getAllLadderCategories();
													while($ladderDetails = $admin->fetch($ladderSQL)) {
												?>
													<option value="<?php echo $ladderDetails['id'] ?>" <?php if(isset($_GET['edit']) and in_array($ladderDetails['id'],$ladderArray)) { echo 'selected'; } ?>><?php echo $ladderDetails['name'] ?></option>
												<?php
													}
												?>
											</select>
