<?php
	...
	$pageName = "Product Sub Varient";
	$parentPageURL = 'sub-varient-master.php';
	$pageURL = 'sub-varient-add.php';
	...
	if(isset($_POST['register'])){
		if($csrf->check_valid('post')) {
			$result = $admin->addSubVarient($_POST, $_FILES);
	...
	if(isset($_GET['edit'])){
		...
		$data = $admin->getUniqueSubVarientById($id);
	}

	if(isset($_POST['id']) && !empty($_POST['id'])) {
		if($csrf->check_valid('post')) {
			$id = trim($admin->escape_string($admin->strip_all($_POST['id'])));
			$result = $admin->updateSubVarient($_POST, $_FILES);
							...
							<div class="card">
								...
								<div class="card-body">
									...
									<div class="form-group row">
										<div class="col-md-4">
											<label>Sub Varient Name <em>*</em></label>
											<input type="text" name="name" value="<?php if(isset($_GET['edit'])) { echo $data['name']; } ?>" class="form-control" required />
										</div>
										<div class="col-md-4">
										...
										<div class="col-sm-3">
											<label>Product Varient<em>*</em></label>
									...
									<div class="form-group row">
										<div class="col-md-8">
											...
											<input type="text" name="description" value="<?php if(isset($_GET['edit'])) { echo $data['description']; } ?>" class="form-control" required />
										</div>
										<div class="col-sm-4">
											<label>Active</label>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			$("#sub_category_id").on("change", function(){
				...
				$.ajax({
					url:"ajaxGetVarientBySubCategoryId.php",
					data:{sub_category_id:sub_category_id},
					type:"POST",
					success: function(response){
						$("#varient_id").html(response);
					},
					error: function(){
						alert("Unable to get content, please try again");
					},
			...
			$("#form").validate({
				...
				rules: {
					name:{
						required:true
						/*remote:{
                           url:"ajax-check-varient-name.php",
                           type: "post",
                           	<?php// if(isset($_GET['edit'])){ ?>
	                           data: {
	                              id: function() {
				...
				messages: {
					name:{
						remote:"Varient name already exists."
					}
				}
			});
