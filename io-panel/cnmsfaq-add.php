<?php
	...
	$pageName = "CNMS FAQ";
	$parentPageURL = 'cnmsfaq-master.php';
	$pageURL = 'cnmsfaq-add.php';
	...
	if(isset($_POST['register'])){
		if($csrf->check_valid('post')) {
			$result = $admin->addCNMSFAQ($_POST, $_FILES);
	...
	if(isset($_GET['edit'])){
		...
		$data = $admin->getUniqueCNMSFAQById($id);
	}

	if(isset($_POST['update']) && !empty($_POST['id'])) {
		if($csrf->check_valid('post')) {
			$id = trim($admin->escape_string($admin->strip_all($_POST['id'])));
			$result = $admin->updateCNMSFAQ($_POST, $_FILES);
						...
						<form action="" id="form" method="post" enctype="multipart/form-data" autocomplete="off">
							<div class="card">
								<div class="card-header">
									<h4 class="card-title mb-0"> CNMS FAQ Details</h4>
								</div>
								<div class="card-body">
