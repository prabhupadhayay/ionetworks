							...
							<tbody>
								<?php
									...
									while($row = $admin->fetch($results)){
										$file_name = str_replace('', '-', strtolower( pathinfo($row['home_image'], PATHINFO_FILENAME)));
										$ext = pathinfo($row['home_image'], PATHINFO_EXTENSION);
										$url =  BASE_URL."/images/product/".$file_name.'_crop.'.$ext;

										$categoryDetails = $admin->getUniqueCategoryById($row['category_id']);
										$subcategoryDetails = $admin->getUniqueSubCategoryById($row['sub_category_id']);

										/*$time=time();
										$permalink = $row['permalink'];
										$permalink = $permalink."-".$time;
										$sqlUpdate = "update ".PREFIX.$tableName." set permalink='".$permalink."' where id='".$row['id']."' ";
										$resultsUpdate = $admin->query($sqlUpdate);*/
								...
								?>
										<tr>
											...
											<td><?php echo $subcategoryDetails['category_name']; ?></td>
											<td><?php if($row['only_for_documentation'] == 0){ echo 'No'; }else{ echo 'Yes'; } ?></td>
											<td>
