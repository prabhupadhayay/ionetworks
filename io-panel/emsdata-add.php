<?php
	...
	$pageName = "EMS Data";
	$parentPageURL = 'emsdata-master.php';
	$pageURL = 'emsdata-add.php';
	...
	if(isset($_POST['register'])){
		if($csrf->check_valid('post')) {
			$result = $admin->addEMSDATA($_POST, $_FILES);
	...
	if(isset($_GET['edit'])){
		...
		$data = $admin->getUniqueEMSDATAById($id);
	}

	if(isset($_POST['update']) && !empty($_POST['id'])) {
		if($csrf->check_valid('post')) {
			$id = trim($admin->escape_string($admin->strip_all($_POST['id'])));
			$result = $admin->updateEMSDATA($_POST, $_FILES);
