<?php
	...
	$pageURL = 'privacy-policy.php';
	$tableName = 'cms_master';

	$sql = "SELECT * FROM ".PREFIX.$tableName." where cms_type='privacy_policy' ";
	$results = $admin->query($sql);
	$data = $admin->fetch($results);

	if(isset($_POST['update'])) {
		if($csrf->check_valid('post')) {
			$result = $admin->updateCMS($_POST, 'privacy_policy', $_FILES);
								...
								<div class="card-body">
									...
									<div class="form-group row">
										<div class="col-sm-12">
											...
											<textarea name="description" rows="7" id="description"><?php echo $data['description']; ?></textarea>
										</div>
									</div>
								</div>
							</div>
