<?php
	...
	$pageName = "Home Testimonial Master";
	$parentPageURL = 'home-testimonial-master.php';
	$pageURL = 'home-testimonial-add.php';
	...
	if(isset($_POST['register'])){
		if($csrf->check_valid('post')) {
			$result = $admin->addHomeTestimonial($_POST, $_FILES);
	...
	if(isset($_GET['edit'])){
		...
		$data = $admin->getUniqueHomeTestimonialById($id);
	}

	if(isset($_POST['id']) && !empty($_POST['id'])) {
		if($csrf->check_valid('post')) {
			$id = trim($admin->escape_string($admin->strip_all($_POST['id'])));
			$result = $admin->updateHomeTestimonial($_POST, $_FILES);
									...
									<div class="form-group row">
										<div class="col-md-4">
											...
											<span class="help-text">
												...
												Images must be exactly <strong>1006 x 549</strong> pixels.
											</span>
											<?php if(isset($_GET['edit'])) {
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$('input[name="image_name"]').change(function(){
				loadImagePreview(this, (1006 / 549));
			});
		});
	</script>
</body>
