<?php
	...
	$pageName = "Why EMS Icon";
	$parentPageURL = 'emsicon-master.php';
	$pageURL = 'emsicon-add.php';
	...
	if(isset($_POST['register'])){
		if($csrf->check_valid('post')) {
			$result = $admin->addEMSIcon($_POST, $_FILES);
	...
	if(isset($_GET['edit'])){
		...
		$data = $admin->getUniqueEMSIconById($id);
	}

	if(isset($_POST['update']) && !empty($_POST['id'])) {
		if($csrf->check_valid('post')) {
			$id = trim($admin->escape_string($admin->strip_all($_POST['id'])));
			$result = $admin->updateEMSIcon($_POST, $_FILES);
						...
						<form action="" id="form" method="post" enctype="multipart/form-data" autocomplete="off">
							<div class="card">
								<div class="card-header">
									<h4 class="card-title mb-0"> EMS Icon Details</h4>
								</div>
								<div class="card-body">
									<div class="form-group row">
										<div class="col-sm-4">
											...
											<span class="help-text">
												...
												Images must be exactly <strong>58 x 55</strong> pixels.
											</span>
											<?php 
											...
											?>
													<br><img src="../images/ems/<?php echo $file_name.'_crop.'.$ext ?>" width="200"  />
											<?php
											} ?>
										</div>
										<div class="col-sm-4">
