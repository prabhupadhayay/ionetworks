<?php
	...
	$pageName = "Home Slider";
	$parentPageURL = 'slider-master.php';
	$pageURL = 'slider-add.php';
	...
	if(isset($_POST['register'])){
		if($csrf->check_valid('post')) {
			$result = $admin->addHomeSlider($_POST, $_FILES);
	...
	if(isset($_GET['edit'])){
		...
		$data = $admin->getUniqueHomeSliderById($id);
	}

	if(isset($_POST['id']) && !empty($_POST['id'])) {
		if($csrf->check_valid('post')) {
			$id = trim($admin->escape_string($admin->strip_all($_POST['id'])));
			$result = $admin->updateHomeSlider($_POST, $_FILES);
								...
								<div class="card-body">
									...
									<div class="form-group row">
										...
										<div class="col-sm-4">
											...
											<input type="file" class="form-control" name="slider_video">
											<span class="help-block" id="dim">(Size: MAX 22MB/ MP4 and WEBM Only`)</span>
											<?php 
												if(isset($_GET['edit']) && !empty($data['slider_video'])) {
											?>
											<br><br>
											<a target="_blank" href="<?php echo BASE_URL; ?>/images/videos/<?php echo $data['slider_video']; ?>">View Video</a><br>
											<?php
												} 
											?>
										</div>
									...
									<div class="form-group row">
										<div class="col-sm-12">
											...
											<input type="text" required class="form-control" name="url" id="" value="<?php if(isset($_GET['edit'])){ echo $data['url']; }?>"/>
										</div>
									</div>
								</div>
							</div>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			$("#form").validate({
				...
				messages: {
					image_name: {
						extension: "Please upload jpg or png or jpeg image"
					}
				}
			});
