<?php
	...
	$pageName = "Ladder Category";
	$parentPageURL = 'ladder-master.php';
	$pageURL = 'ladder-add.php';
	...
	if(isset($_POST['register'])){
		if($csrf->check_valid('post')) {
			$result = $admin->addLadderCategory($_POST);
	...
	if(isset($_GET['edit'])){
		...
		$data = $admin->getUniqueLadderCategoryById($id);
	}

	if(isset($_POST['id']) && !empty($_POST['id'])) {
		if($csrf->check_valid('post')) {
			$id = trim($admin->escape_string($admin->strip_all($_POST['id'])));
			$result = $admin->updateLadderCategory($_POST);
