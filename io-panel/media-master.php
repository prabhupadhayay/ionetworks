<?php
	...
	$pageName = "News & Events Master";
	$pageURL = 'media-master.php';
	$addURL = 'media-add.php';
	$deleteURL = 'media-master.php';
	...
	if(isset($_GET['delId']) && !empty($_GET['delId']) ){
		...
		$admin->deleteMedia($delId);
		header('location: '.$pageURL.'?deletesuccess');
		exit;
	}


	$sqlCMS = "SELECT * FROM ".PREFIX."media_listing_cms ";
	$resultsCms = $admin->query($sqlCMS);
	$data = $admin->fetch($resultsCms);

	if(isset($_POST['update'])) {
		$result = $admin->updateMediaCMS($_POST,$_FILES);
		header("location:".$pageURL."?bannerupdatesuccess");
		exit;
	}
	...
	<div class="main-wrapper">
		...
		<div class="page-wrapper">
			...
			<div class="content container-fluid">
				<?php if(isset($_GET['bannerupdatesuccess'])){ ?>
					<div class="alert alert-success alert-dismissible" role="alert">
						<button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
						<i class="icon-checkmark3"></i> Banner successfully updated.
					</div><br/>
				<?php } ?>
				<div class="row">
					<div class="col-md-12">
						<form action="" id="form" method="post" enctype="multipart/form-data" autocomplete="off">
							<div class="card">
								<div class="card-header">
									<h4 class="card-title mb-0"> News & Events Banner Details</h4>
								</div>
								<div class="card-body">
							...
							<div class="form-actions text-right">
								...
								<button type="submit" name="update" value="update" id="update" class="btn btn-warning"><i class="icon-pencil"></i>Update Banner Details</button>
							</div>
						</form>
					</div>
				</div>
							...
							<tbody>
								<?php
									...
									while($row = $admin->fetch($results)){
										$file_name = str_replace('', '-', strtolower( pathinfo($row['listing_image'], PATHINFO_FILENAME)));
										$ext = pathinfo($row['listing_image'], PATHINFO_EXTENSION);
										$url =  BASE_URL."/images/media/".$file_name.'_crop.'.$ext;
								?>
										<tr>
											<td><?php echo $x++ ?></td>
											...
											<td><?php echo date('d-m-Y', strtotime($row['media_date'])); ?></td>
											<td>
