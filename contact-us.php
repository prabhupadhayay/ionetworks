...
if (isset($_POST['submitContactForm'])) {
    ...
    // Check for existing submission within 24 hours
    if (empty($errors)) {
        $sqlCheck = "SELECT created FROM " . PREFIX . "contact_us_details_modified WHERE email = '" . $email . "' LIMIT 1";
        $queryCheck = $functions->query($sqlCheck);

        if ($functions->num_rows($queryCheck) > 0) {
            $row = $functions->fetch_array($queryCheck);
            $lastSubmissionTime = strtotime($row['created']);
            $currentTime = time();
            $twentyFourHoursAgo = $currentTime - (24 * 60 * 60); // 24 hours in seconds

            if ($lastSubmissionTime >= $twentyFourHoursAgo) {
                // User has submitted the form within the last 24 hours, restrict the submission
                $errors[] = "You have already submitted the form within the last 24 hours. Please try again later.";
            }
        }
    }

    // If there are validation errors, prepare error message
    if (!empty($errors)) {
        $error_message = implode("<br>", $errors);
    }

    if (empty($errors)) {
        $sqlInsert = "insert into " . PREFIX . "contact_us_details_modified(name, lname, mobile, email, country, city, enquiry_type, message)VALUES('" . $name . "', '" . $lname . "', '" . $mobile . "', '" . $email . "', '" . $country . "', '" . $city . "', '" . $enquiry_type . "', '" . $message . "')";
        $queryInsert = $functions->query($sqlInsert);

        //External template 
        include_once("include/emailers/contactus-email.php");

        // Internal email template
        include_once("include/emailers/contactus-emailinternal.php");
        ...
        // Internal email
        $internalRecipient = "iosales@hfcl.com";
        $internalSubject = "Enquiry Received";
        $emailObj->setSubject($internalSubject);
        $emailObj->setEmailBody($internalMsg);
        $emailObj->setAddress($internalRecipient);
        $emailObj->sendEmail();

        //Customer Email 
        $emailObj2 = new Email();
        $emailObj2->setSubject($subject);
        $emailObj2->setEmailBody($invoiceMsg);
        $emailObj2->setAddress($to);
        $emailObj2->sendEmail();
            ...
            <div class="container">
                <div class="row">
                    <div class="col-md-5">
                        ...
                        <?php endif; ?>
                            <form class="formclass" method="POST" id="contactformsumbit">
                                <div class="row">
                                    <div class="enter-input-wrap">
                                        <div class="enterinput-title">
                                            <h6>Name</h6>
                                        </div>
                                    ...
                                    <div class="enter-input-wrap">
                                        ...
                                        <div class="enter-input">
                                            <select name="country" class="form-control checkcountryvalidation">
                                                ...
                                                ?>
                                                    <option <?php if ($countryDetails['country_name'] == '') {
                                                                echo 'selected';
                                                            } ?> value="<?php echo $countryDetails['country_name']; ?>"><?php echo $countryDetails['country_name']; ?></option>
                                                <?php } ?>
                                            </select>
                                        </div>
                                    </div>
                                    ...
                                    <div class="enter-input-wrap">
                                        <div class="enterinput-title" style="opacity: 0;">
                                            <h6>recaptcha</h6>
                                        </div>
                                        <div class="enter-input">
                                    ...
                                    <div class="map-left">
                                        <div class="map-item-inner map-item-inner2">
                                            <ul>
                                                ...
                                                <li>Customer support <p>8792 701 100</p>
                                                </li>
                                            </ul>
                                        </div>
                                        ...
                                        <div class="map-item-inner">
                                            <ul>
                                                ...
                                                <li>Write To Us <p><a href="mailto:iosales@hfcl.com">iosales@hfcl.com</a></p>
                                                </li>
                                            </ul>
                                        </div>
                                    </div>
    ...
    <script type="text/javascript">
        ...
        if ($(window).width() < 800) {
            ...
            ?>
                ...
                $('html, body').animate({
                    scrollTop: $('#<?php echo $_GET['action']; ?>').offset().top - 200
                }, 500);
        ...
        if ($(window).width() > 800) {
            ...
            ?>
                ...
                $('html, body').animate({
                    scrollTop: $('#<?php echo $_GET['action']; ?>').offset().top - 120
                }, 500);
